...
class LunchViewModel  : ViewModel(){
    ...
    private var LunchLiveDataList: MutableLiveData<HotelResponse?> = MutableLiveData()

    fun getLunchLiveDataObserver(): MutableLiveData<HotelResponse?> {
        return LunchLiveDataList
    }

    fun lunchPlace(data: HotelBody) {
        retrofit.lunchPlace(data).enqueue(object : Callback<HotelResponse> {
            override fun onFailure(call: Call<HotelResponse>, t: Throwable) {
                LunchLiveDataList.postValue(null)
            ...
            override fun onResponse(
                ...
                LunchLiveDataList.postValue(response.body())
